...
class FilesystemTree:
    ...
    def build_filesystem_tree(self, curr_dir, lines):
        if not lines:
            return
        if lines[0][0] == '$':
            i = 0
            ...
            if i < len(lines):
                if lines[i][2] == '/':
                    self.build_filesystem_tree(self.root, [*lines[i+1:]])
                elif lines[i][2] == '..':
                    prev_directory = curr_dir.prev
                    self.build_filesystem_tree(prev_directory, [*lines[i+1:]])
                else:
                    next_directory = curr_dir.sub_dirs[lines[i][2]]
                    next_directory.prev = curr_dir
                    self.build_filesystem_tree(next_directory, [*lines[i+1:]])
        else:
            raise Exception("Wrong input!")


file_system = FilesystemTree(lines)
